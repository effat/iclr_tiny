...
from model_SineEncode_iclr import SRL_KT
from utils import *
...
def compute_corr(prob_seq, next_seq, respID_seq, id_to_orgdict,  respID_to_orgDict, USE_dict, item_types):
    ...
    for i in range(0,prob_seq.shape[0]):
        ...
        for  j in range(0,next_seq.shape[1]):
            ...
            ## avoid padding. only add if type is 1
            ...
            for k in range(j+1):
                ...
                #if k > 0: Add type condition. For No_SRL all types are 1
...
def prepare_batches(train_data, batch_size, randomize=True):
   ...
   ## item and labels have first input 0: to predict from the second inputs, as required by the DKT?
    ...
    ## no timestamp
    data = list(zip(item_inputs, label_inputs, type_inputs, item_ids, item_types, resp_ids, qresp_inputs, labels))
...
def train(train_data, val_data, timespan,  model, optimizer, logger, saver, num_epochs, batch_size, grad_clip, id_to_orgDict, respid_to_orgDict, USE_dict):
    ...
    for epoch in range(num_epochs):
        ...
        for item_inputs, label_inputs, type_inputs, item_ids, item_types, resp_ids, qresp_inputs, labels in train_batches:

            rel = compute_corr(item_inputs, item_ids, resp_ids, id_to_orgDict, respid_to_orgDict, USE_dict, item_types)
            ...
            preds, weights = model(item_inputs, label_inputs, type_inputs, item_ids, torch.Tensor(rel), item_types, qresp_inputs)
        ...
        model.eval()
        #### added item_types
        for item_inputs, label_inputs, type_inputs, item_ids,  item_types, resp_ids, qresp_inputs, labels in val_batches:
...
def do_ablation(k, args):
   ### Resp dict in SRL_removed_df folder
    ...
    #test_get_corr(id_to_orgDict, USE_dict)
    ...
    ## ABLATION params [time_include, qrelation_include, qrespCos_include]. All are True initially
    #ablation = {"time_include": True, "qrelation_include": True, "qrespCos_include": True}

    

     
   
    ablation = { "qrelation_include": True}
    #ablation = { "qrelation_include": False}

    ablation_fname =  "  qrelation_include__" + str(ablation["qrelation_include"]) 
    ...
    while True:
        ...
        param_str = str(param_str) + ablation_fname + " fold = " + fold_
    ...
    fname = str(args.savedir) + ablation_fname + " fold = "+ fold_+ "_.txt"
    size_all_mb = (param_size + buffer_size) / 1024**2

    textfile = open(fname, 'w')
    textfile.write("Ablation {}\n".format(ablation_fname))
    ...
    for item_inputs, label_inputs, type_inputs, item_ids,  item_types, resp_ids, qresp_inputs, labels in test_batches:
    ...
    abaltion_str = str(ablation)

    logger.log_dowrite(abaltion_str)
    ...
    logger.close()
    print("Done Fold k = ", k)
    return (auc_test, acc_test, ablation_fname, param_size, size_all_mb)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Train Sinusoidal Positional Encoding_iclr')
    parser.add_argument('--dataset', type=str)
    parser.add_argument('--logdir', type=str, default='./runs/Sine_Positional_Encoding_ICLR')
    parser.add_argument('--savedir', type=str, default='./save/Sine_Positional_Encoding_ICLR')
    ...
    all_accuracy_test = []
    ablation_type = ""
    param_size, size_all_mb = "", ""

    for i in range(start_k, end_k):
        #do_ablation(i, args)
        auc_test, accuracy_test, ablation_fname, param_size, size_all_mb = do_ablation(i, args)
        param_size, size_all_mb = param_size, size_all_mb
    
        all_auc_test.append(auc_test)
        all_accuracy_test.append(accuracy_test)
        ablation_type = ablation_fname

    
    fname = str(args.savedir) + ablation_fname + "_all"+ "_.txt"

    avg_auc, avg_accuracy = np.mean(all_auc_test), np.mean(all_accuracy_test)

    textfile = open(fname, 'w')
    textfile.write("Ablation {}\n".format(ablation_type))

    textfile.write("Average AUC {}\n".format(avg_auc))
    textfile.write("Average Accuracy {}\n".format(avg_accuracy))
