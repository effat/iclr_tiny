...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, rel, l1, l2, timestamp,  pos_key_embeds, pos_value_embeds, mask=None):
...
class SRL_KT(nn.Module):
    def __init__(self, num_items,  embed_size, num_attn_layers, num_heads,
                  max_pos, drop_prob, id_to_orgDict, USE_dict):
        """Knowledge tracing.
        Arguments:
        ...
        #self.respid_to_orgDict = respid_to_orgDict
        
        ### modify: embed size half? comment out relu in forward
        print("init model_iclr")
    ...
    def get_inputs(self, item_inputs, label_inputs, types_, qresp_inputs):
        ...
        ## old code
        #item_inputs = self.item_embeds(item_inputs) ## temp(item_inputs)
        

        #### item_inputs.size() == num rows x timestamp per row x feature per timestamp
        #print("item input", len(item_inputs), "shape ", item_inputs.size(), "data type ", item_inputs.dtype)
        ### ADDED by EF: add type_embedding to item_inputs?
        #item_inputs = item_inputs + types_
        # skill_inputs = self.skill_embeds(skill_inputs)
        ## EF code: expand labels by size embed_size
    ...
    def forward(self, item_inputs, label_inputs, type_inputs, item_ids, rel, timestamp, item_types, qresp_inputs):
        ...
        inputs = self.get_inputs(item_inputs, label_inputs, type_inputs, qresp_inputs)
        ...
        #print("after relu item and label input ", len(inputs), "shape ", inputs.size())
        ...
        #concat_resp = torch.cat([concat_q, resp_embed], dim=-1)
        #output_ = self.final_respout(concat_resp)
